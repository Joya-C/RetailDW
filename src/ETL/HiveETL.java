...
public class HiveETL {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 3) {
            ...
            System.err.println("Usage:HiveETL <input_Dir>  <output_Dir> <srcDir>");
	...
	String srcDir = args[2];
        ...
        try {
            ...
            try (Connection connection = DriverManager.getConnection(CONNECTION_URL, USERNAME, PASSWORD);
                ...
                HiveETL.loadData(outputDir,srcDir,statement);
    ...
    private static void loadData(String InputPath, String srcPath, Statement statement) throws SQLException {
	statement.execute("USE ods_retail");
	String[][] srcDataMappings = {
		{"Customer.csv","ods_retail.ods_customer"},
		{"Product.csv","ods_retail.ods_product"},
		{"Sales.csv","ods_retail.ods_sales"},
		{"Store.csv","ods_retail.ods_store"},
		{"Supplier.csv","ods_retail.ods_supplier"}
	};

	//load data to ODS
	for (String[] mapping : srcDataMappings){
		loadCsvToTempTable(srcPath + "/" + mapping[0], mapping[1], statement);		
	}

	statement.execute("USE dm_retail");
	
        // 定义CSV文件和对应的临时表映射

        String[][] csvToTempTableMappings = {

                {"Customer_dim.csv", "dm_retail.customer_temp_dim"},

                {"Store.csv", "dm_retail.store_temp_dim"},

                {"date_dim.csv", "dm_retail.date_temp_dim"},

                {"Supplier.csv", "dm_retail.supplier_temp_dim"},

                {"Product_dim.csv", "dm_retail.product_temp_dim"},

                {"Sales_dim.csv", "dm_retail.sales_temp_fact"}
        ...
        transferDataToFinalTable("dm_retail.customer_temp_dim", "dm_retail.customer_dim", statement);

        transferDataToFinalTable("dm_retail.store_temp_dim", "dm_retail.store_dim", statement);

        transferDataToFinalTable("dm_retail.supplier_temp_dim", "dm_retail.supplier_dim", statement);

        transferDataToFinalTable("dm_retail.product_temp_dim", "dm_retail.product_dim", statement);

        transferDataToFinalTable("dm_retail.date_temp_dim", "dm_retail.date_dim", statement);
    ...
    private static void transferSalesData(Statement statement) throws SQLException {
        ...
        String salesTransferSql = "from dm_retail.sales_temp_fact " +

                "insert into table dm_retail.sales_fact partition(year = '2023', month = '3') " +

                "select * where date_key <= '20230331' " +

                "insert into table dm_retail.sales_fact partition(year = '2023', month = '4') " +

                "select * where date_key <= '20230430' and date_key >= '20230401' " +

                "insert into table dm_retail.sales_fact partition(year = '2023', month = '5') " +
	...
	//ODS tables
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.store_temp_dim(" +
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.customer_temp_dim(" +
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.supplier_temp_dim(" +
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.product_temp_dim(" +
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.date_temp_dim(" +
        ...
        statement.execute(
                ...
                "create table if not exists dm_retail.sales_temp_fact(" +
    ...
    private static void createDatabase(Statement statement) throws SQLException {
        ...
        statement.execute("CREATE DATABASE dm_retail");
	statement.execute("CREATE DATABASE ods_retail");
